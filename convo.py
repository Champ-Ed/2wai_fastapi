...
class DeepLakePDFStore:
    def __init__(self, path: Optional[str] = None, commit_batch: int = 8, debug: bool = False, dataset_name: Optional[str] = None, org_id: Optional[str] = None):
        # Get org_id from parameter, Streamlit secrets, or environment
        if org_id:
            activeloop_org_id = org_id
        else:
            activeloop_org_id = st.secrets.get("ACTIVELOOP_ORG_ID", os.getenv("ACTIVELOOP_ORG_ID", ""))
        
        # Use configurable dataset name from session or default
        if dataset_name:
            dataset_path = f"hub://{activeloop_org_id}/{dataset_name}"
        else:
            dataset_path = path or f"hub://{activeloop_org_id}/calum_v10"
        ...
        if self.debug:
            logger.debug(f"[DL] Init store path={dataset_path} batch={commit_batch} dataset={dataset_name}")
    ...
    async def rag_query(self, query: str, top_k: int = 5, agent_id_filter: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            # Smart timeout based on connection state
            is_first_query = not hasattr(self, '_connection_warmed')
            timeout_duration = 25.0 if is_first_query else 10.0  # Increased standard timeout from 3s to 10s
            
            if self.debug:
                logger.debug(f"[DL] Using {'first-connection' if is_first_query else 'standard'} timeout: {timeout_duration}s")

            # Guard with per-loop lock
            async with self._loop_lock():
                # Use adaptive timeout for RAG queries
                nodes = await asyncio.wait_for(
                    asyncio.to_thread(_sync_retrieve, query),
                    timeout=timeout_duration
                )
                
                # Mark connection as warmed after successful query
                if is_first_query:
...
class OrchestratedConversationalSystem:
    def __init__(self, session: Dict, agent: str = "1"):
        ...
        try:
            # Use enable_tracing from session if provided, otherwise default to True
            enable_tracing = session.get("enable_tracing", True)
            initialize_client(
            ...
            if self.debug:
                logger.debug(f"LLM client initialized successfully (tracing={enable_tracing})")
        except Exception as e:
        ...
        self.agent = agent
        # Pass dataset_name and org_id from session to DeepLake store
        dataset_name = session.get("dataset_name", "calum_v10")
        org_id = session.get("activeloop_org_id", os.getenv("ACTIVELOOP_ORG_ID", ""))
        self.store = DeepLakePDFStore(commit_batch=8, debug=self.debug, dataset_name=dataset_name, org_id=org_id)
    ...
    def _build_graph(self) -> StateGraph:
        ...
        async def resolve_context_node(state: AgentState) -> AgentState:
            ...
            try:
                # Smart timeout: longer for first connection, shorter for subsequent queries
                # Check if this is likely the first query by seeing if the dataset is "cold"
                is_first_query = not hasattr(self.store, '_connection_warmed')
                timeout_duration = 30.0 if is_first_query else 12.0  # Increased from 5s to 12s to allow for store timeout
                
                if self.debug:
                    logger.debug(f"[NODE] Using {'first-connection' if is_first_query else 'standard'} timeout: {timeout_duration}s")
                
                memories_task = asyncio.create_task(self.store.rag_query(q, top_k=3, agent_id_filter="1"))
                memories = await asyncio.wait_for(memories_task, timeout=timeout_duration)
                
                # Mark connection as warmed after successful query
                if is_first_query:
                    self.store._connection_warmed = True
            except asyncio.TimeoutError:
                if self.debug:
